 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of negative numbers") {
    assert(sum(List(-1,-2,-5,-7,-200)) === -215)
  }

  test("negative and positive sums") {
    assert(sum(List(-2,-5,2, 5, -4, -10, 12, 14)) === 12)
  }

  test("zeroes sum to zero") {
  ...
  test("max of pos and negative numbers") {
    assert(max(List(-3, -5, 3, 2, 1, 9, 0)) === 9)
  }

  test("If list empty, returns error") {
